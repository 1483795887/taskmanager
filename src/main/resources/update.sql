...
select id, 0, 0, start_date
from event;

insert into progress (eid, progress, record, date)
select id, current_progress, current_progress, last_modified_date
from event
on duplicate key update progress = event.current_progress,
                        record   = event.current_progress;
...
insert into progress (eid, progress, record, date)
select id, target_progress, target_progress, start_date
from event
where running = false;
